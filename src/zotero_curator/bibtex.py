...
_BIBTEX_KEY_RE = re.compile(r"@\s*[A-Za-z]+\s*\{\s*([^,\s]+)\s*,")
...
def bbt_item_ids(item_keys: list[str], cfg: CuratorConfig) -> list[str]:
    """Return BBT item identifiers for current library settings."""

    normalized = normalize_item_keys(item_keys)
    if cfg.library_type == "user" and str(cfg.library_id) in {"", "0"}:
        return normalized
    return [f"{cfg.library_id}:{item_key}" for item_key in normalized]
...
def bbt_export_items(
    ...
    if not (cfg.library_type == "user" and str(cfg.library_id) in {"", "0"}):
...
def write_bibtex_export(
    *,
    ...
    if bib_path.exists() and not overwrite:
        raise FileExistsError(f"Export file already exists: {bib_path}")
